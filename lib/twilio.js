...
function PhoneNumber(client, num, opts) {
	...
	logger.debug('Updating Listerner \n');
        logger.debug(update);
...
PhoneNumber.prototype.getNumberDetails = function(fn) {
	...
	logger.debug('Regular Phone Number attrs after get details \n');
	logger.debug(self);
};
...
function OutgoingPhoneNumber(client, num, opts) {
	...
	return function(req, res) {
		...
		// feature detect to see if this is a twiml object
		if(typeof req.body[0].verifyBody != "undefined" ){
		
		    var reqParams = req.body;
		    self.emit(event, reqParams, (new Twiml.Response(res)));
	
		} else {
			// do nothing becuase its not
		}
